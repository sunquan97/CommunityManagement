...
<div class="page-content clearfix">
    <div id="Member_Ratings">
      <div class="d_Confirm_Order_style">
   			<form action="${pageContext.request.contextPath }/searchSecurity.action" method="post" id="search">
	...
	<tbody>
		<c:forEach items="${securitys }" var="security">
			<tr>
				...
				<td style="vertical-align: middle !important;text-align: center;">${security.userid }</td>
				<td style="vertical-align: middle !important;text-align: center;">${security.username }</td>
				<td style="vertical-align: middle !important;text-align: center;">${security.realname }</td>
				<td style="vertical-align: middle !important;text-align: center;">		<c:choose>
          <c:when test="${security.gender ==0 }">
				男
          </c:when>
          <c:otherwise>
				女
        		...
        		<td style="vertical-align: middle !important;text-align: center;">${security.age }</td>
				<td style="vertical-align: middle !important;text-align: center;">${security.phonenumber }</td>
				<td style="vertical-align: middle !important;text-align: center;">${security.address }</td>
				<td style="vertical-align: middle !important;text-align: center;"><fmt:formatDate value="${security.createtime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td style="vertical-align: middle !important;text-align: center;">${security.userrole.rolename }</td>
				<td class="td-status">
		<c:choose>
          <c:when test="${security.flag==0 }">
		...
		<c:choose>
          <c:when test="${security.flag==0 }">
			<a onClick="user_stop(this,${security.userid })"  href="javascript:;" title="停用"  class="btn btn-xs btn-success"><i class="icon-ok bigger-120">停用</i></a> 
          </c:when>
          <c:otherwise>
            <a style="text-decoration:none" class="btn btn-xs " onClick="user_start(this,${security.userid })" href="javascript:;" title="启用"><i class="icon-ok bigger-120">启用</i></a>
          </c:otherwise>
        </c:choose>
					<a title="编辑" onclick="member_edit(${security.userid })" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120">编辑</i></a> 
					<a title="删除" href="javascript:;"  onclick="user_del(this,${security.userid })" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120">删除</i></a>
				</td>
			</tr>
		</c:forEach>
      </tbody>
...
<!--添加用户图层-->
		<form action="${pageContext.request.contextPath }/addSecurity.action" method="post" id="addUserForm" ENCTYPE="multipart/form-data">
	    ...
	    <li><label class="label_name">用户名：</label><span class="add_name"><input name="username" id="username" type="text"  class="text_add"/></span><div class="prompt r_f" id="userbox"></div></li>
	     ...
	     <label><input name="gender" type="radio" class="ace" value="1"><span class="lbl">女</span></label>&nbsp;&nbsp;&nbsp;
			...
			<li><label class="form-label">角&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;色：</label>
			 <span class="select-box" style="width:150px;margin-left:20px;">
				<select class="select" id="userroleid"  name="userroleid" size="1">
					<option name="userroleid" value="1">安保员</option>
					<option name="userroleid" value="11">安保组长</option>
					<option name="userroleid" value="12">安保队长</option>
				</select>
				</span> </li>
...
function user_stop(obj,userid){
	layer.confirm('确认要停用吗？',function(index){
		...
		window.location.href="${pageContext.request.contextPath }/stopSecurity.action?userid="+userid+"" 
	});
}
...
function user_start(obj,userid){
	layer.confirm('确认要启用吗？',function(index){
		...
		window.location.href="${pageContext.request.contextPath }/startSecurity.action?userid="+userid+"" 
	});
}
/*用户-编辑*/
            ...
            if(username== "" || $.trim($("#username").val()).length == 0){
            	...
            	layer.alert('用户名不能含有中文',{
	 			...
	 			icon:1,		
	       	 	...
	       	 	var newUrl = "${pageContext.request.contextPath }/updateSecurity.action?userid="+userid+"";    //设置新提交地址
...
function user_del(obj,userid){
	layer.confirm('确认要删除吗？',function(index){
		...
		window.location.href="${pageContext.request.contextPath }/delSecurity.action?userid="+userid+"" 
	});
}
laydate({
    elem: '#start',
